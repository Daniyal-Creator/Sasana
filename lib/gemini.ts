...
export async function analyzeImage(
  ...
  try {
    const res = await withRetry(() => withTimeout(call(), env.GEMINI_TIMEOUT_MS, "vision"), {
      retries: 1,
      backoffMs: env.GEMINI_RETRY_BACKOFF_MS,
    });
...
export async function askQuestion(
  ...
  try {
    const res = await withRetry(
      () => withTimeout(call(), env.GEMINI_TIMEOUT_MS, "chat"),
      { retries: 1, backoffMs: env.GEMINI_RETRY_BACKOFF_MS },
    );
